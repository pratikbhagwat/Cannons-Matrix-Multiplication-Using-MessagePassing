...
public class NewMasterWorkerProcess {
     ...
     * @throws IOException
     */
    public NewMasterWorkerProcess(String matrix1Filename,String matrix2Filename,int numberOfSpawnedProcesses) throws IOException {
    ...
    public void doTheJob(){
        ...
        for (int iteration = 0;iteration<(int)Math.sqrt(totalNumberOfProcesses); iteration++){ // do the procedire ndim number of times.
...
//            this.matrix2.set(i,replacement);
            List<Double> part1 = this.matrix1.get(i).subList(0,chunkSize);
            List<Double> part2 = this.matrix1.get(i).subList(chunkSize,this.matrix1.get(i).size());
            part2.addAll(part1);
            ...
            part1 = this.matrix2.get(i).subList(0,chunkSize);
            part2 = this.matrix2.get(i).subList(chunkSize,this.matrix2.get(i).size());
            part2.addAll(part1);
        ...
        for (int i = 0;i<this.chunkSize;i++){
            for (int j =0;j<this.chunkSize;j++){
                for (int k = 0;k<this.chunkSize;k++){
                    this.answerMatrix.get(i).set(j, this.answerMatrix.get(i).get(j) + this.matrix1.get(i).get(k) * this.matrix2.get(j).get(k));
                }
            }
        }
    }
...
//        }
        ...
        for (int i =1;i<(int)Math.sqrt(totalNumberOfProcesses);i++){
            for (int rowNumber=chunkSize*i;rowNumber<chunkSize*i+chunkSize;rowNumber++){
                List<Double> part1 = this.matrix1.get(rowNumber).subList(0,i*chunkSize);
                List<Double> part2 = this.matrix1.get(rowNumber).subList(i*chunkSize,this.matrix1.size());
                part2.addAll(part1);
                ArrayList<Double> newArraylist = new ArrayList<>();
                newArraylist.addAll(part2);
                this.matrix1.set(rowNumber,newArraylist);
            }
        }

        for (int i =1;i<(int)Math.sqrt(totalNumberOfProcesses);i++){
            for (int rowNumber=chunkSize*i;rowNumber<chunkSize*i+chunkSize;rowNumber++){
                List<Double> part1 = this.matrix2.get(rowNumber).subList(0,i*chunkSize);
                List<Double> part2 = this.matrix2.get(rowNumber).subList(i*chunkSize,this.matrix2.size());
                part2.addAll(part1);
                ArrayList<Double> newArraylist = new ArrayList<>();
                newArraylist.addAll(part2);
                this.matrix2.set(rowNumber,newArraylist);
            }
        }
